...
/**
  ...
  */
case class DyingCat(_x: Float, _y: Float, _width: Float, _height: Float, initLife: Long) extends ElementOfPower {
  ...
  override def render(g: Graphics, dxx: Int, dyy: Int): Unit = {
    if (alive) {
      ...
      g2d.drawImage(Util.dyingCatImage, x + dxx toInt, y + dyy toInt, width toInt, height toInt, null)
      g2d.dispose()
    }
  }
}
